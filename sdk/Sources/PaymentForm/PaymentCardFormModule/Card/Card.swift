...
public struct Card {
    ...
    public static func makeCardCryptogramPacket(cardNumber: String, expDate: String, cvv: String, merchantPublicID: String, publicKey: String, keyVersion: Int) -> String? {
        ...
        guard let cryptogramData = try? RSAUtils.encryptWithRSAPublicKeyNewMethod(str: decryptedCryptogram, pubkeyBase64: publicKey) else {
            return nil
        }
        ...
        let browserInfoBase64 = self.makeBrowserInfoBase64()
        let object = CryptogramType(CardInfo: cardInfo, version: convertKeyVersion, value: cryptogramString, browserInfoBase64: browserInfoBase64)
    ...
    private static func makeBrowserInfoBase64() -> String? {
        ...
        let info = BrowserInfo(
            ...
            Width: String(width),
            TimeZone: String(timeZoneOffsetMinutes),
    ...
    private static func makeCardCryptogramPacket(with cardNumber: String, expDate: String, cvv: String, merchantPublicID: String) -> String? {
        ...
        guard let publicKey = oldPublicKey(), let cryptogramData = try? RSAUtils.encryptWithRSAPublicKeyNewMethod(str: decryptedCryptogram, pubkeyBase64: publicKey) else {
            return nil
        }
        let cryptogramString = RSAUtils.base64Encode(cryptogramData)
            .replacingOccurrences(of: "\n", with: "")
    ...
    public static func makeCardCryptogramPacket(with cvv: String) -> String? {
        guard let publicKey = oldPublicKey(), let cryptogramData = try? RSAUtils.encryptWithRSAPublicKeyNewMethod(str: cvv, pubkeyBase64: publicKey) else {
            return nil
        }
        let cryptogramString = RSAUtils.base64Encode(cryptogramData)
            .replacingOccurrences(of: "\n", with: "")
